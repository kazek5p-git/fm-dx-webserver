...
///////////////////////////////////////////////////////////////
	...
	const SignalStrengthUnit = 'dBf'; // This value is automatically updated via the config file
    ...
    async function fetchFirstLine() {
        ...
        try {
            ...
            if (!version) {
                ...
                version = /^\d/.test(firstLine) ? firstLine : "Unknown"; // Check if first character is a numbe
            }
...
function normalizeSensitivity(Sensitivity, SignalStrengthUnit) {
    ...
    return Math.round(result); // Always round to the nearest whole numbe
}
        ...
        if (eventData.type === 'Scanner') {
            ...
            if (ScanButton) {
                if (Scan === 'off') {
                    ...
                    ScanButton.setAttribute('aria-pressed', 'true');
...
function BlinkAutoScan() {
    ...
    if (parentElement) {
        ...
        blinkText.setAttribute('role', 'status');
        blinkText.setAttribute('aria-live', 'assertive');
        blinkText.setAttribute('aria-atomic', 'true');
    ...
    function ScannerButtons(Sensitivity, ScannerMode, ScanHoldTime) {
        ...
        ScannerButton.setAttribute('aria-pressed', 'false');
        ...
        ScannerButton.addEventListener('click', function() {
            ...
            if (!isLongPress) {
				if (isActive) {
                    ...
                    ScannerButton.setAttribute('aria-pressed', 'true');
...
function toggleScan(isLongPressAction) {
    ...
    function createScannerControls(Sensitivity, ScannerMode, ScanHoldTime) {
        ...
        const VolumeSliderWidth = VolumeSlider.clientWidth; // Get the width of the volume slide
 ...
 if (ScanPE5PVBstatus) {
        ...
        const volumeSliderParent = document.getElementById('volumeSlider').parentNode;
        volumeSliderParent.style.display = 'none'; // Hide volume slide
    ...
    function initializeDropdown(container, logPrefix, commandPrefix, Sensitivity, ScannerMode, ScanHoldTime) {
        ...
        const dropdownId = `scanner-dropdown-${commandPrefix.toLowerCase()}`;
    ...
    function closeAllDropdowns() {
        ...
        allDropdowns.forEach(dropdown => {
            dropdown.style.display = 'none';
            const parentContainer = dropdown.closest('.scanner-dropdown');
            const dropdownInput = parentContainer?.querySelector('input');
            if (dropdownInput) {
