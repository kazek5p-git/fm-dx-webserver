/*
    ...
    https://github.com/AmateurAudioDude/FM-DX-Webserver-Plugin-S-Mete

    Original concept by Analog Signal Meter: https://github.com/NO2CW/FM-DX-Webserver-analog-signal-mete
*/
...
(() => {
  ...
  const OutsideField = true;                  // Display meter outside the SIGNAL containe
  ...
  const radioNoiseFloor = -119;               // The reported dBm signal reading with no antenna connected used to calibrate enableLowSignalInterpolation
  ...
  function initSignalMeterSmall() {
      document.addEventListener('DOMContentLoaded', function() {
          ...
          if (!existsPeakmeter && setMeterLocation === 'auto') signalMeter.style.backdropFilter = 'blur(5px)'; // Blur used in FM-DX Webserve
          signalMeter.style.width = '256px';
          signalMeter.style.height = '13px';
          // Setting of pixelated was required if height was an odd numbe
          signalMeter.style.imageRendering = 'auto';
          // Configure squelch marke
          const markerCanvas = document.createElement('canvas');
          markerCanvas.id = 'signal-meter-small-marker-canvas';
          markerCanvas.setAttribute('tabindex', '0');
          markerCanvas.setAttribute('role', 'button');
          markerCanvas.setAttribute('aria-label', 'Signal meter and squelch marker. Press Enter to show or hide the meter.');
          ...
          function drawMarker() {
              if (isEnabledSquelch && markerPositionMin && showMarker) {
                  ...
                  if (useThemeColors) {
                    ...
                    markerCtx.strokeStyle = colorBackgroundMarker; // Marker colo
                  } else {
          ...
          if (isEnabledSquelch) {
              ...
              function startDragging(event) {
                  event.preventDefault(); // Prevent default touch behavio
                  ...
                  // Disable middle click from setting marke
              ...
              markerCanvas.addEventListener('wheel', function(event) {
                  event.preventDefault(); // Prevent default scrolling behaviou
          ...
          function toggleSignalMeterVisibility() {
              const currentOpacity = signalMeter.style.opacity;
              signalMeter.style.opacity = currentOpacity === '1' ? '0' : '1';
              markerCanvas.style.opacity = currentOpacity === '1' ? '0' : '1'; // Hide squelch marke
              showMarker = true;
          ...
          markerCanvas.addEventListener('dblclick', function(event) {
              ...
              if (isMouseDoubleClickWithin) {
                  toggleSignalMeterVisibility();
                  isMouseDoubleClickWithin = false; // Reset double click
              }
          });

          markerCanvas.addEventListener('keydown', function(event) {
  ...
  function drawSignalMeter(signalValue, signalValueHighest, ctx, backgroundImage, signalMeter) {
      ...
      ctx.beginPath();
      ctx.moveTo(minMeterPosition, 0); // Start from the top left corne
      ...
      if (enableLowSignalInterpolation) {
        ...
        */
        ...
        let sIntValue = 18; // Value in px of the interpolated noise floo
