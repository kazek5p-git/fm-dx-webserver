...
////////////////////////////////////////////////////////////
    ...
    function makeKeyboardActivatable(element, label) {
        ...
        if (element.dataset && element.dataset.a11yKeyBound === 'true') return;
        if (element.dataset) {
    ...
    function enhanceStationListAccessibility(container) {
        if (!container) return;

        const interactiveNodes = container.querySelectorAll('td, a');
        interactiveNodes.forEach((node) => {
            const nodeTagName = node.tagName ? node.tagName.toLowerCase() : '';
            const isAnchorWithoutHref = nodeTagName === 'a' && !node.getAttribute('href');
            const hasClickHandler = typeof node.onclick === 'function';
            const hasPointerCursor = node.style?.cursor === 'pointer';
            const isInteractive = isAnchorWithoutHref || hasClickHandler || hasPointerCursor;

            if (isInteractive) {
                const label = node.getAttribute('title') || node.textContent.trim() || 'Activate LiveMap action';
                makeKeyboardActivatable(node, label);
            }
        });
    }
...
initializeWrapperPosition();
    ...
    function createToggleButton() {
        ...
        makeKeyboardActivatable(toggleButton, 'Toggle station list');
    ...
    makeKeyboardActivatable(closeButton, 'Close LiveMap');
...
function receiveGPS() {;	
	...
	async function displayStationData(data, txposLat, txposLon, picode, pol, foundPI) {
        ...
        if (!stationListContainer) {
            ...
            stationListContainer.style.visibility = 'visible'; 
            stationListContainer.setAttribute('role', 'region');
            stationListContainer.setAttribute('aria-label', 'LiveMap station list');
            document.body.appendChild(stationListContainer);

            if (!stationListContainer.dataset.a11yObserverAttached) {
                const a11yObserver = new MutationObserver(() => {
                    enhanceStationListAccessibility(stationListContainer);
                });
                a11yObserver.observe(stationListContainer, { childList: true, subtree: true });
                stationListContainer.dataset.a11yObserverAttached = 'true';
            }
        } else {
            stationListContainer.style.left = `${iframeContainer.offsetLeft}px`;
            stationListContainer.style.top = `${iframeContainer.offsetTop + iframeContainer.offsetHeight}px`;
        ...
        // Add the table with stations to the station list containe
		...
		cityCells.forEach(cell => {
			...
			if (picode !== '?' && foundPI) {
				...
				// Filter and sort the stations of the selected city by ERP in descending orde
    ...
    async function openOrUpdateIframe(picode, freq, stationid, station, city, distance, ps, itu, pol, radius) {
        ...
        function createAndInsertIframe() {
            ...
            if (!iframeContainer) {
                ...
                iframeContainer.appendChild(toggleButton); // Append the toggle button to the containe
	...
	makeKeyboardActivatable(frequencyElement, 'Tune to previously received frequency');
    ...
    async function handleWebSocketMessage(event) {
        try {
            ...
            if (freq !== previousFreq) {
                if (frequencyElement) {
                    ...
                    if (freq_save !== null) {
                        ...
                        existingDiv.style.transform = 'translate(-50%, -50%)'; // Adjust position back to cente
                    }
            ...
            console.error("Error processing the message:", error); // Log any errors that occu
        }
    }
      ...
      if (typeof addIconToPluginPanel === 'function') {
        ...
        const buttonObserver = new MutationObserver(() => {
          ...
          if ($pluginButton.length > 0) {
            ...
            // Once the button is found and set up, disconnect the buttonObserve
...
createButton('LIVEMAP-on-off');
	...
	// Function to check if the user is logged in as an administrato
